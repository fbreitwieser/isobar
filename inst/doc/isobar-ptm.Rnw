%\VignetteIndexEntry{Isobar's PTM Modules}
%\VignetteDepends{}
...
\evensidemargin=.05in
\headheight=-.3in
\newcommand{\isobar}{\emph{isobar}}
\newcommand{\isobarPtm}{\emph{isobar}$^{\sc ptm}$}

\title{\isobar{} for quantification of PTM datasets}
...
given proper input files using the script \texttt{report/create_reports.R}.
...
Reports will be generated calling \texttt{path_to_isobar/report/create_reports.R --peptide} from the directory containing the peaklists, identifications and \texttt{properties.R}.
...
After calling PhosphoRS (\texttt{java -jar phosphoRS.jar phosphors.in.xml phosphors.out.xml}), the resulting \textsc{xml} file can be read:
...
\texttt{mascotParser2.pl} and thresholded (\emph{e. g.} \texttt{--minDeltaScore=10}). For CSV identification files which contain all hits for each spectrum (not just the best one), the function \texttt{calc.delta.score} within the R package is provided.
...
other experiments can be integrated using the property \texttt{compare.to.quant} in \texttt{properties.R}:
