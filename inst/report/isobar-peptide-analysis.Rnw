...
protein.group <- proteinGroup(report.env$ibspectra)
df.pg <- unique(.proteinGroup.as.concise.data.frame(protein.group)[,c("peptide","proteins")])
...
all.peps <- ddply(report.env$quant.tbl,c("peptide","modif","proteins"),
                  function(d) c(n.spectra=ifelse(any(!is.na(d[,"n.spectra"])),max(d[,"n.spectra"],na.rm=T),NA)))
