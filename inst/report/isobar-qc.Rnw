...
\subsection*{Reporter intensities and Normalization}
Box-plot of the reporter intensities before and after the normalization process. Isobar applies a normalization factor to impose equal median or summed intensity in each channel. Some channels may be excluded, as defined in \texttt{properties.R}.
...
#"\\multicolumn{2}{c}{after n.\\footnote{Intensities before and after normalization}}",
       ...
       paste(names(n.factors),
             ...
             sprintf("%.2f",colMeans(ri.nn,na.rm=TRUE)),
             sprintf("%.2f",apply(ri.nn,2,median,na.rm=TRUE)),
...
gg <- grid.arrange(g,
tableGrob(ddply(quant.tbl,c('channels','class2','class1')[c('channels','classes') %in% colnames(quant.tbl)],
