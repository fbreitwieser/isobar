...
<<fig-reporterIntensityPlot,fig=T,echo=F,width=2.5,height=2.3,results=hide>>=
 print(reporterIntensityPlot(ibspectra) + .gg_theme(legend.position="bottom"))
@
\end{minipage}
\hfill
...
#  scale_x_continuous("Ratio",breaks=log10(breaks),labels=breaks) +
  ...
  .gg_theme(legend.position="none",
       axis.text.x = .gg_element_text(colour="grey50"))
...
#  scale_x_continuous("Ratio",breaks=log10(breaks),labels=breaks) +
  scale_x_continuous(expression(paste(log[10]," ratio"))) +
  theme_bw(base_size=10) +
  .gg_theme(legend.position="none",axis.text.x = .gg_element_text(colour="grey50"))  + scale_fill_brewer(palette="Set1")
