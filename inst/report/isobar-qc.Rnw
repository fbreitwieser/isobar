  ...
  if (!is.null(attr(ratiodistr,"method"))) {
     ...
     classTxt <- sapply(names(t),function(class) 
                        sprintf("\\emph{%s} (tags %s)",class,
                                paste(tagNames[classLabels==class],collapse=", ")))
     if (m == "intraclass") {
...
png("fig_maplot.png",width=9,height=4,units="in",res=600,pointsize=14)
maplot(ibspectra,noise.model=noise.model,bty="l",pairs=FALSE)
dev.off()
@
