...
\paragrap{Individual ratio distributions}
<<reporter-intensities-2,fig=T,echo=F,width=6.5,height=4.5,results=hide>>=
...
tableGrob(ddply(quant.tbl,c('channels','class2','class1')[c('channels','classes') %in% colnames(quant.tbl)],
           ...
           gpar.coretext = gpar(fontsize=8), gpar.coltext=gpar(fontsize=8), gpar.rowtext=gpar(fontsize=8)),
ncol=1))
