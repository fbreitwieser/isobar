...
\title{Isobaric Tag Data Analysis Report\\ \analysisname}
...
cmbn <- t(unique(protein.tbl[,c("r1","r2")]))
...
if (!is.null(cl)) {
  ...
  for (i in seq_along(reporterNames(ibspectra))) {
    cat(reporterNames(ibspectra)[i]," & ",ifelse(is.na(cl[i]),"",cl[i]),"\\\\ \n")
  }
  cat("\\end{tabular}\n")
}
@
...
\emph{protein} & Protein ID.  \\
\emph{peptides} & Number of peptides protein was identified with. \\
\emph{spectra} & Number of spectra protein was identified with. \\
\emph{quant} & Number of spectra a quantitative information was available for \textit{ch2} versus \textit{ch1}. \\
...
if (is.single.comparision) {
  cat("\\section{Significantly Regulated Proteins ",cmbn[2,1],"/",cmbn[1,1],"}\n\n")
} else {
  cat("\\section{Significantly Regulated Proteins}\n\n")
}


if (any(protein.tbl$is.significant == 1,na.rm=TRUE)) {
  
  cat("\\small")
  print.longtablehdr(coldef.s,!is.single.comparision,ncol.p-1,FALSE)
  draw.hline <- FALSE
  
  for (cmb_i in seq_len(ncol(cmbn))) {
    sel <- protein.tbl$r1==cmbn[1,cmb_i] &
           protein.tbl$r2==cmbn[2,cmb_i] &
           protein.tbl$is.significant == 1
    sel[is.na(sel)] <- FALSE
    if (any(sel)) {
      if (draw.hline) {
        cat (" \\hline\n")
      ...
      proteins <- protein.tbl[sel,"protein"][order(protein.tbl[sel,"lratio"])]
      for (protein in proteins) {
        prot.info <- my.protein.infos[[protein]]
        protein.row <- protein.tbl[protein.tbl$protein==protein & sel,,drop=FALSE]
        reporter.peptides <- peptides(pg,protein=protein)
        n.spectra <- length(names(spectrumToPeptide(pg))[spectrumToPeptide(pg)%in%reporter.peptides])
        cat("\n")
        cat(" \t",sprintf("\\hyperref[protein.%s]{%s}",
            ...
            " & ",n.spectra,
            " & ",protein.row[1,"n.spectra"])
        if (!is.single.comparision) {
            cat(" & ",cmbn[1,cmb_i]," & ",cmbn[2,cmb_i])
        }
            cat(" & ",sprintf("\\textbf{%.2f}",10^protein.row[1,"lratio"]),
            " & ",sep=" ")
        cat(draw.protein.ratio2(protein.row[1,],bnd,diff=TRUE,ratiodistr=ratiodistr))
        cat(" \\\\  \n")
        prot_i <- prot_i + 1
      }
    }
...
<<echo=FALSE,results=tex>>=
if (is.single.comparision) {
  cat("\\section{All Proteins Relative Expression ",cmbn[2,1],"/",cmbn[1,1],"}")
} else {
  cat("\\section{All Proteins Relative Expression}")
}
@

<<allproteins.init,echo=FALSE,results=hide>>=

proteins.n.group <- unique(protein.tbl[,c('protein','group')])
proteins <- proteins.n.group$protein[order(proteins.n.group$group)]
...
cat("\\small")
print.longtablehdr(coldef,!is.single.comparision,ncol.p,TRUE)

proteins.notquantified <- c()

message("Writing protein quantifications table ...")
for (protein in proteins) {
  protein.rows <- protein.tbl[protein.tbl$protein==protein,,drop=FALSE]
...
#  protein.name.i <- unique(protein.tbl$latex.protein.name.2[protein.tbl$protein==protein])
  ...
  draw.protein.group(protein.group,reporter.protein.g)
  prot_i <- prot_i + 1
