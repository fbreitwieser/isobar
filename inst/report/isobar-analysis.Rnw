...
start.file <- function(fname,use.sink=FALSE) {
  ## sink() does not work with Sweave 
  if (isTRUE(use.sink))
    sink(fname)
  else
    cat(sprintf("\\begin{filecontents}{%s}\n",fname))
}

end.file <- function(use.sink=FALSE) {
  if (isTRUE(use.sink))
    sink()
  else
    cat(sprintf("\\end{filecontents}\n"))
}
...
cat("\\secsubtitle{Number of significant proteins: ",paste(sprintf("%s/%s: %s",sanitize(n.quant[["class2"]]),sanitize(n.quant[["class1"]]),n.quant[["V1"]]),collapse="; "),"}\\vspace{.5cm}\n" )


if (any(quant.tbl[["is.significant"]] == 1,na.rm=TRUE)) {  
  start.file("sign-proteins.tex")
  ...
  cat("\\small\n")
  cat("\\LTXtable{\\textwidth}{sign-proteins.tex}\n")
  cat("\\normalsize\n")
...
cat("\\secsubtitle{Number of quantified proteins: ",paste(sprintf("%s/%s: %s",sanitize(n.quant[["class2"]]),sanitize(n.quant[["class1"]]),n.quant[["V1"]]),collapse="; "),"}\\vspace{.5cm}\n")

start.file("protein-quant.tex")
...
cat("\\small\n")
cat("\\LTXtable{\\textwidth}{protein-quant.tex}\n")
cat("\\normalsize\n")
...
n.quant <- ddply(quant.tbl,c("class1","class2"),function(x) sum(is.na(x[["lratio"]])))
cat("\\secsubtitle{Number of identified but not quantified proteins: ",paste(sprintf("%s/%s: %s",sanitize(n.quant[["class2"]]),sanitize(n.quant[["class1"]]),n.quant[["V1"]]),collapse="; "),"}\\vspace{.5cm}")


start.file("protein-notquant.tex")
cat("\\begin{longtable}{rXrrr}",
"  \\#",
"  & \\textbf{protein}",
"  & \\rh{group}",
"  & \\rh{peptides}",
...
start.file("protein-groups.tex")
cat("\\begin{longtable}{rlXp{6cm}}\n")
prot_i <- 1
