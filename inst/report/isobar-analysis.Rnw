...
\title{Isobaric Tag Analysis Report\\ \analysisname}
...
if (any(quant.tbl$is.significant == 1,na.rm=TRUE)) {  
  cat("\\small")
  print.longtablehdr("protein",coldef.s,!is.single.comparision,ncol.p-1,FALSE)
  draw.hline <- FALSE
...
cat("\\small")
print.longtablehdr("protein",coldef,!is.single.comparision,ncol.p,TRUE)
...
for (protein in proteins.notquantified) {
  protein.rows <- quant.tbl[quant.tbl$protein==protein,,drop=FALSE]
  protein.rows <- protein.rows[order(protein.rows$r1,protein.rows$r2),,drop=FALSE]
  protein.groupnumber <- protein.rows[1,"group"]
  prot.info <- my.protein.infos[[protein]]
